...
public class JedisUtils {
     ...
     * 执行延迟队列doing最大等待时间
     */
    private static long queue_maxWait;
    /**
     * #离线消息最大的缓存时间
     */
    private static long msg_maxWait;
    /** 操作Key的方法 */
    ...
    private static final String FILE_NAME = "/properties/redis.properties";

    private JedisUtils() {}

    public static void  initPool(){
        queue_maxWait = Long.parseLong(PropertiesUtils.getProperty(FILE_NAME,"redis.maxWaitMillis"));
        msg_maxWait = Long.parseLong(PropertiesUtils.getProperty(FILE_NAME,"redis.maxWaitMillis"));
        JedisPoolConfig config = new JedisPoolConfig();
        config.setMaxTotal(Integer.parseInt(PropertiesUtils.getProperty(FILE_NAME,"redis.maxTotal")));
        config.setMaxIdle(Integer.parseInt(PropertiesUtils.getProperty(FILE_NAME,"redis.minIdle")));
        config.setMaxWaitMillis(Long.parseLong(PropertiesUtils.getProperty(FILE_NAME,"redis.maxWaitMillis")));
        config.setTestOnBorrow(Boolean.parseBoolean(PropertiesUtils.getProperty(FILE_NAME,"redis.testOnBorrow")));
        config.setTestOnReturn(Boolean.parseBoolean(PropertiesUtils.getProperty(FILE_NAME,"redis.testOnReturn")));
        //redis如果设置了密码：
        /*jedisPool = new JedisPool(config,
                PropertiesUtils.getProperty(FILE_NAME,"redis.ip"),
                Integer.parseInt(PropertiesUtils.getProperty(FILE_NAME,"redis.port")),
                10000,
                PropertiesUtils.getProperty(FILE_NAME,"redis.password"));*/
        //redis未设置了密码：
        jedisPool = new JedisPool(config,
                PropertiesUtils.getProperty(FILE_NAME,"redis.host"),
                Integer.parseInt(PropertiesUtils.getProperty(FILE_NAME,"redis.port")));
    }
    ...
    private static class LazyHolder {
        private static final JedisUtils redisPoolClient = new JedisUtils();
    }
    /**
    ...
    public static synchronized Jedis getJedis() {
        try {
            if (JedisUtils.getPool() != null) {
                Jedis resource = JedisUtils.getPool().getResource();
                return resource;
            } else {
                //redis,3个小时没有数据传输就会断开长连接,这里是为了重新建立长连接
                initPool();
                return JedisUtils.getPool() != null ? JedisUtils.getPool().getResource() : null;
            }
        } catch (Exception e) {
    ...
    public static long getQueue_maxWait(){
        try {
            if (JedisUtils.getPool() == null) {
                initPool();
            }
        ...
        return getInstance().queue_maxWait;
    }
    public static long getMsg_maxWait(){
        try {
            if (JedisUtils.getPool() == null) {
                initPool();
            }
        ...
        return getInstance().msg_maxWait;
    }
    /**
     * 设置过期时间
    ...
    public class Keys {
        ...
        public long ttl(String key) {
            ...
            Jedis sjedis=getInstance().getJedis();;
    ...
    public class SortSet {
        ...
        public long zadd(String key, Map<String,Double> scoreMembers) {
            Jedis jedis = getInstance().getJedis();
            //String json = Jsons.toJson(scoreMembers);
    ...
    public class Lists {
         ...
         * 在value的相对位置插入记录
         * @param key
         * @param where LIST_POSITION   前面插入或后面插入
         * @param pivot 相对位置的内容
         * @param value 插入的内容
         * @return 记录总数
         * */
