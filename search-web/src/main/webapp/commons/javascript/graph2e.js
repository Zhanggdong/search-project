...
var _g_LineColor=new Array( '#88df76', '#58A2C7', '#58A2C7');
var _g_FillColor=new Array( '#268c1f', '#156290', '#156290');
var _g_FillColor2=new Array('#58c745', '#3BA4D1', '#3BA4D1');
var _s_temp = 2014;
...
var _g_nBarAreaHeight = 125;
var _g_nBarMaxHeight = 100;
...
function genInitContent(){
	...
	// 生成初始框架表格
	document.body.innerHTML = "<table id='tab_full' cellspacing='0' cellpadding='0' style='background:none;border:none; width:100%; height:100%; border-collapse:collapse;'><tr><td colspan='2' height=''><div id='idDiv' name='idDiv' style='border: solid 0px red;width:100%;height:158px;'></div></td></tr><tr><th><table cellspacing='0' cellpadding='0' class='tb_left'><th class='th_0' onClick='' style=''>"+titl+"</th></table></th><td id='td_top' style='vertical-align:top' height='30px'>&nbsp;</td></tr><tr><td id='td_left' style='vertical-align:top' width='"+_g_titleColWidth+"px'>&nbsp;</td><td id='td_main' style='vertical-align:top'>&nbsp;</td></tr></table><div id='div_top' style='border:0px solid red;position:absolute;width:100px;height:30px; overflow-x:hidden; overflow-y:hidden;border:solid 0px #FF0000'></div><div id='div_left'  style='position:absolute;width:100px;height:100px;overflow-y:hidden; overflow-x:hidden;border:solid 0px #0000FF'></div><div id='div_main' style='position:absolute;width:100px;height:100px; overflow:hidden' onscroll='scrollSync()'></div>";
	genLeftTable();
	var oFull = document.getElementById("tab_full");
	var winlen =oFull.offsetWidth;	
	_oldfultablen = winlen;
...
function genMainTable(){
	...
	for(var i=1; i<_g_rowsCount; i++)
	{
		...
		for(var j=1; j<_g_colsCount; j++)
		{
			...
			{
				s.push("<td style='width:"+_onelen+"px'><table cellspacing='0' cellpadding='0' class='tb_in'><tr><td colspan='3' class='td_u' style='width:"+_onelen+"px'>"+_g_arr_totalHtml[i][j]+"</td></tr><tr><td class='td_l' id='"+bjtd+"'>"+_g_arr_subHtml[i][j][0]+"</td><td class='td_m' id='"+cctd+"'>"+_g_arr_subHtml[i][j][1]+"</td><td class='td_r' id='"+cstd+"' style='display:none'>"+_g_arr_subHtml[i][j][2]+"</td></tr></table></td>");
			}
...
function genLeftTable()
{
	...
	var str = "";
	var s=[];
	s.push("<table cellspacing='0' cellpadding='0' class='tb_left' id='tab_left'>");
	for(var i=1; i<_g_rowsCount; i++)
	{
		str = _g_arr_totalHtml[i][0];
		if(_g_isSwitched)
		{
			str = "<img src='images/blank.gif'><span onClick=\"sortTableRow("+i+")\" style='cursor:hand'>"+str+"</span>";
		}
		else
		{
			lv_str = "";
			...
			for(var j=0; j<lv; j++)
				lv_str += "<img src='images/blank.gif'>";

			if(_g_arrRowIsParent[i-1]==1)
				str = lv_str+"<img id='pm_"+i+"' src='images/minus.gif' style='cursor:hand' onClick=\"showHide(this,"+(i-1)+")\"><span onClick=\"sortTableRow("+i+")\" style='cursor:hand' title='点击排序'>"+str+"</span>";
		...
		s.push("<tr><th style='display:none;' class='th_n'>"+str+"</th></tr>");
	}
	s.push("</table>");
...
_g_isProcing = true;
	...
	if(!_g_isSwitched)
	{
		for(var i=1; i<_g_colsCount-1; i++)
		{
			...
			{
			//	if(_g_arr_subData[row][i][2]==_g_arr_subData[row][j][2]) //超时数相等
				if(true) //超时数相等
				{
				...
				{
					if( (_g_oldSortDirect==0 && _g_arr_subData[row][i][2] > _g_arr_subData[row][j][2]) || (_g_oldSortDirect==1 && _g_arr_subData[row][i][2] < _g_arr_subData[row][j][2]) )
					{
						switchCol(i, j);
						bIsNeedUpdate = true;
					}
...
function drawBarBackground()
{
	...
	for(var i=1; i<6; i++)
	{
		...
		_g_barContent+="<v:Textbox style='POSITION:absolute;Z-INDEX:3003;LEFT:"+(nLeft-45)+";TOP:"+(nTop-8)+";FONT-SIZE:12;COLOR:#000000' inset='5pt,5pt,5pt,5pt' print='t'>"+(_g_maxValue/5*i)+"</v:Textbox>";
		//nTop -= 61;
		nTop -= 5;
		_g_barContent+="<v:line style='POSITION:absolute;Z-INDEX:3002;' strokecolor='#f0f0f0' from='"+(nLeft-0)+"px,"+nTop+"px' to='"+(nLeft+nWidth-16)+"px,"+nTop+"px'/>";
	}
	_g_barContent+="<v:rect style='width:125px;height:"+(_g_nBarMaxHeight+20)+";Z-INDEX:3001;LEFT:"+(nLeft)+";TOP:"+(_g_nBarAreaHeight-_g_nBarMaxHeight-20)+";POSITION:absolute;' strokecolor='#d0d0d0' filled='t' fillcolor='#FFFFFF'/>";
	
	
	//var _s_temp = document.getElementById('s_year');
	//var _s_temp="";
	_g_barContent+="<v:Textbox title='' style='POSITION:absolute;Z-INDEX:9010;LEFT:85;TOP:127;FONT-SIZE:12;COLOR:#000000;width:22;text-align:center;' inset='5pt,5pt,5pt,5pt' print='t'>共受理</v:Textbox>";
	_g_barContent+="<v:Textbox title='' style='POSITION:absolute;Z-INDEX:9010;LEFT:145;TOP:127;FONT-SIZE:12;COLOR:#000000;width:22;text-align:center;' inset='5pt,5pt,5pt,5pt' print='t'>共办结</v:Textbox>";
	_g_barContent+="<v:Textbox title='' style='POSITION:absolute;Z-INDEX:9010;LEFT:0;TOP:125;FONT-SIZE:12;COLOR:#000000;width:22;text-align:center;' inset='5pt,5pt,5pt,5pt' print='t'>从"+_s_temp+"年至今</v:Textbox>";
...
function addBar(left,top,height,colorType, percent,clickcol)
{
	...
	if(height>2)
	{
		_g_barContent+="<v:rect title='"+percent+"' style='width:"+_g_nBarWidth+";height:"+(height-2)+";Z-INDEX:3008;LEFT:"+(left+1)+";TOP:"+(top+1)+";POSITION:absolute;' strokecolor='"+_g_LineColor[colorType]+"' filled='t' fillcolor='"+_g_FillColor[colorType]+"'>"+
		"<v:fill type='gradient' opacity='0.8' color2='"+_g_FillColor2[colorType]+"' angle='90' focus='50%' /></v:rect>";
	}
	if(height>0)
	{
		_g_barContent+="<v:rect title='"+percent+"' style='width:"+(_g_nBarWidth+2)+";height:"+(height-1)+";Z-INDEX:3009;LEFT:"+left+";TOP:"+top+";POSITION:absolute;' strokecolor='"+_g_FillColor[colorType]+"' filled='f'></v:rect>";
	}
}
function addText(left,top,str,title,clickcol)
{
	...
	if(left<-40 || left>_g_arr_offsetData[0][2]-8-_g_nBarWidth-_g_titleColWidth)
		return;
	left = left + _g_titleColWidth-8;
	_g_barContent+="<v:Textbox title='"+title+"' onclick='' style='POSITION:absolute;cursor: pointer;Z-INDEX:3010;LEFT:"+left+";TOP:"+top+";FONT-SIZE:12;COLOR:#000000;width:"+(_g_nBarWidth+3)+";text-align:center;' inset='5pt,5pt,5pt,5pt' print='t'>"+str+"</v:Textbox>";
...
// 当前选中行_g_oldSortRow
	...
	for (j=1; j < _g_colsCount; j++)
	{
		...
		b = _g_nBarAreaHeight;
		for(k=0; k<_g_arr_subData[_g_oldSortRow][j].length-1; k++)
		{
			if(arr_totle[j]<=0)
			{
				arr_totle[j] = 0;
